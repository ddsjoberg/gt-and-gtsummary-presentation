...
- {gt} is wonderful, and we ar doing the most brief introduction to it.  
...
- {gt} documentation is so organized and color coded.  it's easy to find what you're looking for.
---
# {gt} installation
...
- use `vars()` because we can select multiple columns.  Also accepts character vector of names
...
---
class: center
background-image: url(images/gtsummary-HexSticker.png)
background-size: contain
...
- summarize data first!  you will often catch mistakes.  Data is complicated, and udnerstanding it up front is important.
...
- Add column indicating statistics presented.
    - footnote about stats performed is gone!
]
]
.pull-right[
...
- but does not inlcude reference groups
...
- also great with time-to-event endpoint when you cannot do a `tbl_summary()`
...
- You've probaby seen something like this before

- It's a Kaplan-Meier curve.  It shows the probabilty of being free from an event (e.g. cancer recurrence after treatment)
