...
- package has sets of functions for modifying each pace of a table
...
- build the regression model on your own....we are not in the business of mode estimation or checking

- let's review some examples with code
---
# {gtsummary} summarize models with tbl_regression()
...
.pull-right[
```{r}
pluck(tbl_summary_1, "gt_calls") %>% head()
```
]
