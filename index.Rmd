...
.pull-right[
.large[For brevity, we'll use an abbreviated version of the trial data set with few columns.]
...
It's natural that a user of the {gtsummary} package would want to customize the aesthetics of the table with the many {gt} features available.  Every function in {gt} is available to use on a {gtsummary} object.
...
- Resulting tables can be merged (i.e. side-by-side tables for models with the same predictors and different outcomes).  Tables can also be stacked on top of one another.
...
.pull-right[
<p align="center"><img src="images/tbl_survival_1.png" width=95%></p>
]
